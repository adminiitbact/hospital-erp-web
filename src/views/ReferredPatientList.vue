  ...
  <div class="columns-4 w-row">
    ...
    <div class="column w-col w-col-10">
      ...
      <div class="hospital-profile-header-div _1">
        <h1 class="heading-2 _3">Referred Patient List</h1>
        <search-component :placeholder="'Search Patient Name'"
        :rows="allPatients" :searchIndex="1"
        v-on:finish="updatePatientListRows($event)"></search-component>
      </div>
...
export default class ReferredPatientList extends Vue {
  patientListKey=0;
  ...
  mounted() {
    this.$store.dispatch('fetchReferredPatients');
  }

  get allPatients() {
    const rows = [];
    this.referredPatients.forEach((patient) => {
